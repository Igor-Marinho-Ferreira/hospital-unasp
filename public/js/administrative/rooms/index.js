...
function setEventSubmitInFormCreate() {
    if (form = document.getElementById("formRoomsCreate"))
...
async function handleRequestStatus(id, enableOrDisable) {
    const completeUrl = `/administrative/rooms/update-status?status=${enableOrDisable}&id=${id}`;
 ...
 */
 async function fillModal(userId) {
    ...
    let modalCode = document.getElementById("codeUpdate");
    let modalDescription = document.getElementById("descriptionUpdate");
    let modalStatus = document.getElementById("statusUpdate");

    const response = await fetch(`${BASE_URL}/administrative/rooms/find/${userId}`);
 ...
 */
 async function handleDelete(id) {
    const completeUrl = `/administrative/rooms/delete?id=${id}`;
    const response = await fetch(`${BASE_URL}${completeUrl}`);
