...
abstract class BaseRepository
{
     ...
     */
    public function get(bool $showingQuery = false): array|object|null
    {
        if ($showingQuery) dd($this->queryString);
        return $this?->results;
    }

    public function getLastInsertedId()
    {
        return $this->select(['id'])->order('id','DESC')->limit(1)?->done()?->get()?->id;
    }
}
