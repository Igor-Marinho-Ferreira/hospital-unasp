...
/**
 * Method responsible for adding CNPJ in the URL for editing
 * @param string $key
 * @param string $value
 * @return string
 */
function addParamUrl(string $key, string $value): string
{
    $url = CURRENT_URI;
    $explode = explode("?", $url);
    $simple = $explode[0] ?? '';
    ...
    return $simple . "?" . http_build_query($params);
}


/**
 * Responsible for minimizing the letters of an array
 * @return ?array
 */
function array_lower_keys(array $array): ?array
{
    $return = [];
    foreach ($array as $key => $value) {
